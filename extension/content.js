...
(function () {
  ...
  // 戻り値: { type:'folder'|'file'|'unknown', id:string } | null
  ...
  // 失敗を null に潰すと「なぜ DOM フォールバックに落ちたか」が画面から追えず、
  // 「ローカルパスが見つかりません」しか出ないため原因究明ができない。
  // 成功時は { breadcrumbs, isFolder, name }、失敗時は { error, code } を返す。
  async function getBreadcrumbsByApi() {
    const ref = getDriveRefFromUrl();
    if (!ref) {
      return { error: "URL から Drive の ID を取得できません", code: "NO_DRIVE_REF" };
    }
    return new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage(
          { type: "apiResolvePath", folderId: ref.id, isFileId: ref.type === "file" },
          (resp) => {
            if (chrome.runtime.lastError) {
              const msg = chrome.runtime.lastError.message;
              dlog("apiResolvePath lastError", msg);
              resolve({ error: msg, code: "SENDMESSAGE_FAILED" });
              return;
            }
            if (resp && resp.ok && resp.breadcrumbs && resp.breadcrumbs.length) {
              resolve({
                breadcrumbs: resp.breadcrumbs,
            ...
            resolve({
              error: (resp && resp.error) || "API がパスを返しませんでした",
              code: (resp && resp.code) || "API_EMPTY",
            });
          }
        );
      } catch (e) {
        dlog("apiResolvePath threw", e);
        resolve({ error: String(e && e.message || e), code: "EXCEPTION" });
      }
    });
  }
  ...
  //   { breadcrumbs: string[], altCandidates: string[][],
  //     source: "api"|"show-path"|"dom-title"|"file-name-only"|"unresolved-open-id",
  //     driveRef: object|null, fileName: string|null,
  //     apiError: string|null, apiErrorCode: string|null }
  // apiError / apiErrorCode は API で解決できなかった理由。DOM フォールバックに
  // 落ちた場合でも上位 (popup / 通知) が原因を提示できるように必ず載せる。
  async function getFullBreadcrumbsDetailed() {
    const urlRef = getDriveRefFromUrl();
    let apiError = null;
    ...
    try {
      ...
      if (api && api.breadcrumbs && api.breadcrumbs.length) {
        const driveRef = { type: api.isFolder ? "folder" : "file", id: urlRef.id };
        const fileName = api.isFolder ? null : api.name;
        dlog("getFullBreadcrumbs: via API", api.breadcrumbs);
        return {
      ...
      if (api && api.code) {
        ...
        dlog("getFullBreadcrumbs: API failed", apiErrorCode, apiError);
      }
    } catch (e) {
      apiError = String(e && e.message || e);
      apiErrorCode = "EXCEPTION";
      dlog("API path resolve threw", e);
    }

    // ファイル単体URLで API 解決失敗時のフォールバック: title からファイル名のみ返す
    if (urlRef && urlRef.type === "file") {
    ...
    // open?id=... は Drive の UI が無いページなので DOM からも何も取れないことがある。
    // その場合だけ専用 source を返し、上位で「サインインが必要」と案内させる
    // (API 以外に解決手段が無いため)。DOM から取れたならそのまま使う。
    if (primary.length === 0 && urlRef && urlRef.type === "unknown") {
      return {
        breadcrumbs: [],
        source: "unresolved-open-id",
        altCandidates: [],
        driveRef: urlRef,
        fileName: null,
        apiError,
  ...
  window.addEventListener("__DTE_FETCH_PATH__", async () => {
    ...
    try {
      ...
      result = {
        ...
        driveRef: d.driveRef || null,
        fileName: d.fileName || null,
        apiError: d.apiError || null,
        apiErrorCode: d.apiErrorCode || null,
        url: location.href,
      };
    } catch (e) {
