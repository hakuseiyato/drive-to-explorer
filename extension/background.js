...
const CONTEXT_MENU_LINK_ID = "drive-to-explorer-open-link";
...
function apiErrorNote(info) {
  ...
  if (!code) return "";
  if (SIGNIN_FIXABLE_CODES.includes(code)) {
    return (
  ...
  return (
    "\n\nDrive API 解決失敗 (" + code + ")" +
    (info.apiError ? ": " + info.apiError : "") +
    "\nサインインでは解決しません。" +
    "オプション画面の「API モードをテスト」で詳細を確認してください。"
  );
}
...
async function openCurrentFolder(tab) {
  ...
  if (
    ...
    (
      info.source === "unresolved-open-id" ||
      (info.driveRef && info.driveRef.type === "unknown")
    )
  ) {
    const msg =
      "このリンク形式 (open?id=...) の解決には Drive API へのサインインが必要です。\n" +
      "オプション画面で「サインイン」を実行してください。";
    notify("サインインが必要", msg);
    return { ok: false, error: msg };
  }
  ...
  if (info.driveRef && info.driveRef.type === "file") {
    ...
    const parentBc = bc.length && bc[bc.length - 1] === fileName
      ? bc.slice(0, -1)
      : bc;
    const rf = await runOpen({
      kind: "file",
      name: fileName,
      breadcrumbs: parentBc,
      altCandidates: info.altCandidates || [],
    });
    if (rf && !rf.ok) return { ...rf, error: (rf.error || "") + apiErrorNote(info) };
    return rf;
  }

  if (!info.breadcrumbs || info.breadcrumbs.length === 0) {
    const msg = "Drive のフォルダ階層を取得できませんでした。" + apiErrorNote(info);
    notify("エラー", msg);
    return { ok: false, error: msg };
  }
  ...
  // 解決に失敗したうえ API も使えていない場合は、原因を error に添える
  if (r && !r.ok) return { ...r, error: (r.error || "") + apiErrorNote(info) };
  return r;
}

async function openFromLinkUrl(linkUrl) {
  const patterns = [
    /\/file\/d\/([^/?#]+)/,
    /\/folders\/([^/?#]+)/,
    // docs.google.com はアカウント指定 /u/<n>/ が挟まることがある。
    // /forms/d/e/<formId>/viewform は公開フォーム用の別 ID 体系なので除外する。
    /\/(?:document|spreadsheets|presentation|forms|drawings)\/(?:u\/\d+\/)?d\/(?!e\/)([^/?#]+)/,
    /[?&]id=([^&#]+)/,
  ];
  let id = null;
  for (const pattern of patterns) {
    const match = String(linkUrl || "").match(pattern);
    if (match) {
      id = match[1];
      break;
    }
  ...
  let d;
  try {
    d = await DTE_API.getFolderPathDetailedCached(id, {});
  } catch (e) {
    if (e && e.code === "NEEDS_INTERACTIVE") {
      const msg =
        "リンクからの解決には Drive API へのサインインが必要です。" +
        "オプション画面で「サインイン」を実行してください。";
      notify("サインインが必要", msg);
      return { ok: false, error: msg };
    }
    if (e && e.code === "NO_CLIENT_ID") {
      const msg =
        "OAuth Client ID を読み取れませんでした。\n" +
        "オプション画面の「Drive REST API (OAuth)」セクションを確認してください。";
      notify("設定エラー", msg);
      return { ok: false, error: msg };
    }
    // 403 (権限なし) / 404 (無効な ID) / ネットワーク断などは
    // サインインしても解決しないので、API のエラー本文をそのまま見せる
    const msg = String(e && e.message || e);
    notify("リンクの解決に失敗しました", msg);
    return { ok: false, error: msg };
  }

  if (d.isFolder === true) {
    return await runOpen({ kind: "current", breadcrumbs: d.path });
  }
  return await runOpen({
    kind: "file",
    name: d.name,
    breadcrumbs: d.path.slice(0, -1),
  });
}
...
async function runOpen(target) {
  ...
  // 共有ドライブ名の補完フォールバック
  // Drive API の drives.get は drive.metadata.readonly スコープでは 403 になり、
  // 共有ドライブ名が breadcrumbs の先頭から欠落することがある
  // (files.get は「ドライブ」等の汎用ラベルしか返さないため代替にならない)。
  // その場合ローカルの <root>\共有ドライブ\* を列挙し、各ドライブ名を挟んだ
  // 候補を作って再試行する。API に依存しないので確実に当たる。
  if (!foundPath && !fallbackParent && breadcrumbs.length > 0) {
    const sub = breadcrumbs
      .map((s) => (s || "").trim())
      .filter((s) => s.length > 0)
      .join("\\");
    const prefixes = ["共有ドライブ", "Shared drives"];
    const listPaths = [];
    for (const r of localRoots) {
      const root = String(r).replace(/[\\/]+$/, "");
      for (const pre of prefixes) listPaths.push(root + "\\" + pre);
    }
    const listed = await sendToHost({ action: "list_subdirs", paths: listPaths });
    if (listed && listed.ok) {
      const expanded = [];
      for (const entry of listed.results || []) {
        for (const dir of entry.dirs || []) {
          expanded.push(entry.path + "\\" + dir + "\\" + sub);
        }
      }
      if (expanded.length) {
        const expandedFinal =
          target.kind === "file" || target.kind === "folder"
            ? expanded.map((b) => (target.name ? b + "\\" + target.name : b))
            : expanded;
        const retry = await sendToHost({
          action: "exists_many",
          paths: expandedFinal,
        });
        if (retry && retry.ok) {
          const hit = (retry.results || []).find((r) => r.exists);
          if (hit) {
            foundPath = hit.path;
          } else if (action === "select") {
            // 本体が無い場合は親フォルダ (共有ドライブ名込み) を探す
            const parentRetry = await sendToHost({
              action: "exists_many",
              paths: expanded,
            });
            const ph =
              parentRetry && parentRetry.ok
                ? (parentRetry.results || []).find((r) => r.exists)
                : null;
            if (ph) fallbackParent = ph.path;
          }
        }
      }
    } else if (listed && listed.error) {
      hostErrors.push(`list_subdirs: ${listed.error}`);
    }
  }
      ...
      chrome.contextMenus.create(
        {
          ...
          // ページ用の項目と同名にすると、Drive 上でリンクを選択した状態の
          // 右クリックで同名項目が 2 つ並んで区別できなくなる
          title: "このリンクをエクスプローラーで開く",
          contexts: ["link"],
          targetUrlPatterns: [
...
// ---- 初回インストール時のローカルルート自動設定 ------------------------
// 未経験者が最も詰まるのが「Drive for desktop のドライブレターを自分で調べて
// 入力する」工程。Native Host の detect_roots で自動検出して埋めておく。
// 既に設定済みなら何もしない (ユーザーの手動設定を上書きしない)。
// 1 度成功したら二度と走らせないためのフラグ。
// これが無いと、ユーザーが意図的にルートを空にしても起動のたびに書き戻してしまう。
const AUTO_DETECT_DONE_KEY = "localRootsAutoDetectDone";

async function autoDetectLocalRoots() {
  const { [AUTO_DETECT_DONE_KEY]: done } =
    await chrome.storage.local.get(AUTO_DETECT_DONE_KEY);
  if (done) return;
  const existing = await getLocalRoots();
  if (existing.length > 0) {
    // 既に手動設定済み。以後は自動検出しない
    await chrome.storage.local.set({ [AUTO_DETECT_DONE_KEY]: true });
    return;
  }
  // Native Host 未登録 (拡張ロード直後 / ブラウザ再起動前) では必ず失敗する。
  // フラグを立てずに戻り、onStartup の再試行に賭ける。
  const resp = await sendToHost({ action: "detect_roots" });
  if (!resp || !resp.ok) return;
  const roots = (resp.roots || []).map((r) => r.path).filter(Boolean);
  if (roots.length === 0) return;
  await chrome.storage.sync.set({ localRoots: roots, localRoot: roots[0] });
  await chrome.storage.local.set({ [AUTO_DETECT_DONE_KEY]: true });
  notify(
    "ローカルルートを自動設定しました",
    roots.join(" / ") + "\n変更するにはオプション画面を開いてください。"
  );
  updateBadge();
}

// onInstalled だけだと、setup.bat の手順 (Host 登録 → 起動中ブラウザに拡張をロード →
// ブラウザ再起動) では拡張ロード時点で Host 未登録のため必ず失敗する。
// onStartup でも試行して、ブラウザ再起動後に確実に設定されるようにする。
chrome.runtime.onInstalled.addListener(() => {
  autoDetectLocalRoots().catch(() => {});
});
chrome.runtime.onStartup.addListener(() => {
  autoDetectLocalRoots().catch(() => {});
});
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === CONTEXT_MENU_LINK_ID) {
    openFromLinkUrl(info.linkUrl);
  }
    ...
    try {
      ...
      if (msg.type === "openTargetWithBreadcrumbs") {
        ...
        if (
          bc.length === 0 &&
          (
            msg.source === "unresolved-open-id" ||
            (msg.driveRef && msg.driveRef.type === "unknown")
          )
        ) {
          const error =
            "このリンク形式 (open?id=...) の解決には Drive API へのサインインが必要です。\n" +
            "オプション画面で「サインイン」を実行してください。";
          notify("サインインが必要", error);
        ...
        if (isFile) {
          ...
          // popup 側でも apiErrorHint を出すが、通知にも原因を残す
          sendResponse(
            r && !r.ok ? { ...r, error: (r.error || "") + apiErrorNote(msg) } : r
          );
          return;
        }
        const r = await runOpen({
      ...
      if (msg.type === "apiResolvePath") {
        try {
          const d = await DTE_API.getFolderPathDetailedCached(msg.folderId, {
