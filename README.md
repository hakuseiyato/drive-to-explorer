...
Drive API へのサインインは**任意**です。未サインインでも Drive 画面の解析でフォルダを開けます。ただし次の機能は API が必須です。

| 機能 | サインイン |
|---|---|
| `/drive/folders/<id>` を開いているときにフォルダを開く | 不要（画面解析で動作） |
| ファイル単体 URL (`/file/d/<id>/view`) からファイルを選択して開く | **必須** |
| `open?id=<id>` 形式のリンクを開く | **必須** |
| 他サイト上の Drive・Google ドキュメントのリンクを右クリックで開く | **必須** |
| 深い階層・共有ドライブの正確な解決 | 推奨（画面解析では取りこぼす場合あり） |

**どちらの手順を踏むかは、あなたが誰かで決まります。**

#### ケース A: 自分で OAuth Client ID を発行する（一般の利用者はこちら）

本リポジトリには既定の OAuth Client ID が同梱されていますが、その同意画面は Google Cloud 上で **「テスト」ステータス**で運用しています。テストユーザーとして登録されていない Google アカウントでは、**既定 Client ID ではサインインできません**（作者の GCP プロジェクトのクォータが第三者の利用で消費されるのを防ぐためです）。

そのため一般の利用者は、自分の Client ID を発行してください。**オプション画面の「セットアップウィザードを開く」** に従うのが最短です（各ステップで Cloud Console の正しいページへ直接ジャンプし、リダイレクト URI は自動でクリップボードにコピーされます）。

手順の詳細は [docs/GCP_SETUP.md](docs/GCP_SETUP.md) と、後述の「A. Google Cloud Console で OAuth クライアントを作成」「B. 拡張オプションに登録」を参照してください。
（リダイレクト URI は `https://pkiecgchhhhcgnjjgofmlfobbhacdcge.chromiumapp.org/` 固定です）

#### ケース B: 既定 Client ID をそのまま使う（テストユーザーに登録済みの方）

配布元からテストユーザーに登録してもらっている場合は、ボタン 1 つで完了します。
...
> **「このアプリは確認されていません」と警告が出る場合**: 「詳細」 → 「Drive to Explorer に移動」で進みます。Google の審査を通していない自作アプリの標準的な警告で、問題ありません。
>
> **「アクセスをブロック: ～ は Google の審査プロセスを完了していません」等で進めない場合**: そのアカウントがテストユーザーに登録されていません。ケース A の手順で自分の Client ID を発行してください。
...
>
> ただし `manifest.key` を同梱しているため第三者が同じ拡張機能 ID を再現でき、既定 Client ID を使われると作者の GCP プロジェクトのクォータを消費します。そのため既定 Client ID は OAuth 同意画面を「テスト」ステータスで運用しています。**テストユーザーに登録されていないアカウントではサインインできません**（テストユーザーの上限は 100 人で、アプリの全期間で累計されます）。
>
