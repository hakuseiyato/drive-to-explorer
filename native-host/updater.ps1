...
function Write-Status {
  ...
  try {
    ...
    # BOM なし UTF-8 で書く。Windows PowerShell 5.1 の
    # `Set-Content -Encoding UTF8` は BOM を付けてしまい、これを読む Python 側の
    # json.load が BOM で失敗して進捗・完了が拡張に届かなくなる。
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText(
      $StatusFile, ($obj | ConvertTo-Json -Compress), $utf8NoBom
    )
  } catch { }
}
...
try {
  ...
  # ルート直下ファイル
  foreach ($f in @('README.md', 'update.ps1', 'update.bat', 'setup.ps1', 'setup.bat')) {
    $s = Join-Path $src $f
    if (Test-Path $s) {
